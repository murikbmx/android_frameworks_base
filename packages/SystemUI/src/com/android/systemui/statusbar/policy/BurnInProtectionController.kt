...
class BurnInProtectionController @Inject constructor(
...
) : NavigationModeController.ModeChangedListener, ConfigurationListener {
    ...
    private val shiftInterval = context.resources.getInteger(
        R.integer.config_statusBarBurnInProtectionShiftInterval
    ) * 1000L
    ...
    fun startShiftTimer() {
        ...
        shiftJob = coroutineScope.launch {
            while (isActive) {
                ...
                val nbOffset = Offset(
                    getBurnInOffset(maxNavBarOffsetX), getBurnInOffset(maxNavBarOffsetY)
                )
                logD {
